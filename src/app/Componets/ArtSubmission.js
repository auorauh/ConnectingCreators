...
const ArtSubmission = ({ children, onDismiss, file, className = '' }) => {
  ...
  const submitArt = async (e) => {
    ...
    const webhookURL = process.env.NEXT_PUBLIC_ART_SUBMISSION_WEBHOOK;
...
const messageData = {
  ...
  embeds: [{
    ...
    fields: [
      ...
      { name: "Link", value: formData.artUrl || "No URL provided" },
  ...
  if (file || childFile) {
    ...
    try {
      ...
      onDismiss();
      setFormData({ artist: '', title: '', artUrl: '', description: '', instagram: '' });
    } catch (err) {
